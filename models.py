...
class GCNN_with_descriptors(nn.Module):
    def __init__(self, n_output=1, num_features_pro=1024, output_dim=128, dropout=0.2, descriptor_dim=80, transformer_dim=32, nhead=4, num_layers=2, dim_feedforward=128):
        ...
        print('GCNN_with_descriptors Loaded')
        self.n_output = n_output
        self.pro1_conv1 = GCNConv(num_features_pro, num_features_pro)
        self.pro1_fc1 = nn.Linear(num_features_pro, output_dim)
        self.pro2_conv1 = GCNConv(num_features_pro, num_features_pro)
...
class GCNN_mutual_attention(nn.Module):
    def __init__(self, n_output=1, num_features_pro=1024, output_dim=128, dropout=0.2, descriptor_dim=80, transformer_dim=32, nhead=4, num_layers=2, dim_feedforward=128):
        ...
        print('GCNN_mutual_attention Loaded')
        self.n_output = n_output
        self.pro1_conv1 = GCNConv(num_features_pro, num_features_pro)
        self.pro1_fc1 = nn.Linear(num_features_pro, output_dim)
        self.pro2_conv1 = GCNConv(num_features_pro, num_features_pro)
...
class GCNN_desciptors_only(nn.Module):
    def __init__(self, n_output=1, num_features_pro=1024, output_dim=128, dropout=0.2, descriptor_dim=80, transformer_dim=128, nhead=8, num_layers=2, dim_feedforward=256):
        super(GCNN_desciptors_only, self).__init__()
        print('GCNN_desciptors_only Loaded')
        self.n_output = n_output
        self.transformer_dim = transformer_dim
    ...
    def forward(self, pro1_data, pro2_data, mas1_straight, mas1_flipped, mas2_straight, mas2_flipped, mas1_straight_lengths, mas1_flipped_lengths, mas2_straight_lengths, mas2_flipped_lengths):
        pro1_data = None # just to make sure :)
        pro2_data = None
...
class GCNN_graph_only(nn.Module):
    def __init__(self, n_output=1, num_features_pro=1024, output_dim=128, dropout=0.2, transformer_dim=128, nhead=8, num_layers=2, dim_feedforward=256):
        super(GCNN_graph_only, self).__init__()
        print('GCNN_graph_only Loaded')
        self.n_output = n_output
        self.transformer_dim = transformer_dim
    ...
    def forward(self, pro1_data, pro2_data, mas1_straight, mas1_flipped, mas2_straight, mas2_flipped, mas1_straight_lengths, mas1_flipped_lengths, mas2_straight_lengths, mas2_flipped_lengths):
        ...
        mas2_straight = None
        mas2_flipped = None
        mas1_straight_lengths = None
        mas1_flipped_lengths = None
        mas2_straight_lengths = None
        mas2_flipped_lengths = None
        
        batch_size = pro1_data.batch.max() + 1
...
# AttGNN Variations - Using Graph Attention Networks as backbone
...
class AttGNN_desc_pool(nn.Module):
    def __init__(self, n_output=1, num_features_pro=1024, output_dim=128, dropout=0.2, descriptor_dim=80, heads=1):
        super(AttGNN_desc_pool, self).__init__()
        print('AttGNN_desc_pool Loaded')
        self.n_output = n_output
        self.hidden = 8
        self.heads = heads
...
class AttGNN_with_descriptors(nn.Module):
    def __init__(self, n_output=1, num_features_pro=1024, output_dim=128, dropout=0.2, descriptor_dim=80, transformer_dim=32, nhead=4, num_layers=2, dim_feedforward=128, heads=1):
        super(AttGNN_with_descriptors, self).__init__()
        print('AttGNN_with_descriptors Loaded')
        self.n_output = n_output
        self.hidden = 8
        self.heads = heads
...
class AttGNN_mutual_attention(nn.Module):
    def __init__(self, n_output=1, num_features_pro=1024, output_dim=128, dropout=0.2, descriptor_dim=80, transformer_dim=32, nhead=4, num_layers=2, dim_feedforward=128, heads=1):
        super(AttGNN_mutual_attention, self).__init__()
        print('AttGNN_mutual_attention Loaded')
        self.n_output = n_output
        self.hidden = 8
        self.heads = heads
...
class AttGNN_geom_transformer(nn.Module):
    def __init__(self, n_output=1, num_features_pro=1024, output_dim=128, dropout=0.2, descriptor_dim=80, transformer_dim=128, nhead=8, num_layers=2, dim_feedforward=256, heads=1):
        super(AttGNN_geom_transformer, self).__init__()
        print('AttGNN_geom_transformer Loaded')
        self.n_output = n_output
        self.transformer_dim = transformer_dim
        self.hidden = 8
        self.heads = heads
        
        # GAT layers - similar to other models
        self.pro1_conv1 = GATConv(num_features_pro, self.hidden * 16, heads=self.heads, dropout=dropout)
        self.pro1_fc1 = nn.Linear(self.hidden * 16 * self.heads, output_dim)
        self.pro2_conv1 = GATConv(num_features_pro, self.hidden * 16, heads=self.heads, dropout=dropout)
        self.pro2_fc1 = nn.Linear(self.hidden * 16 * self.heads, output_dim)
    ...
    def forward(self, pro1_data, pro2_data, mas1_straight, mas1_flipped, mas2_straight, mas2_flipped, mas1_straight_lengths, mas1_flipped_lengths, mas2_straight_lengths, mas2_flipped_lengths):
        # Process protein 1 with GAT - similar logic as other models
        pro1_x, pro1_edge_index, pro1_batch = pro1_data.x, pro1_data.edge_index, pro1_data.batch
        pro1_nodes = self.pro1_conv1(pro1_x, pro1_edge_index)
        pro1_nodes = self.relu(pro1_nodes)
        # Apply linear layer to each node (instead of global pooling)
        ...
        # Process protein 2 with GAT - similar logic as other models  
        pro2_x, pro2_edge_index, pro2_batch = pro2_data.x, pro2_data.edge_index, pro2_data.batch
        pro2_nodes = self.pro2_conv1(pro2_x, pro2_edge_index)
        pro2_nodes = self.relu(pro2_nodes)
        # Apply linear layer to each node (instead of global pooling)
...
class AttGNN_descriptors_only(nn.Module):
    def __init__(self, n_output=1, num_features_pro=1024, output_dim=128, dropout=0.2, descriptor_dim=80, transformer_dim=128, nhead=8, num_layers=2, dim_feedforward=256, heads=1):
        super(AttGNN_descriptors_only, self).__init__()
        print('AttGNN_descriptors_only Loaded')
        self.n_output = n_output
        self.transformer_dim = transformer_dim
    ...
    def forward(self, pro1_data, pro2_data, mas1_straight, mas1_flipped, mas2_straight, mas2_flipped, mas1_straight_lengths, mas1_flipped_lengths, mas2_straight_lengths, mas2_flipped_lengths):
        ...
        for i in range(batch_size):            
            ...
            # Combine all elements for this sample: descriptors only
            sequence = torch.cat([
                mas1_straight_sample,
                mas1_flipped_sample,
                mas2_straight_sample,
...
class AttGNN_graph_only(nn.Module):
    def __init__(self, n_output=1, num_features_pro=1024, output_dim=128, dropout=0.2, transformer_dim=128, nhead=8, num_layers=2, dim_feedforward=256, heads=1):
        super(AttGNN_graph_only, self).__init__()
        print('AttGNN_graph_only Loaded')
        self.n_output = n_output
        self.transformer_dim = transformer_dim
        self.hidden = 8
        self.heads = heads
        
        # GAT layers - same as other models
        self.pro1_conv1 = GATConv(num_features_pro, self.hidden * 16, heads=self.heads, dropout=dropout)
        self.pro1_fc1 = nn.Linear(self.hidden * 16 * self.heads, output_dim)
        self.pro2_conv1 = GATConv(num_features_pro, self.hidden * 16, heads=self.heads, dropout=dropout)
        self.pro2_fc1 = nn.Linear(self.hidden * 16 * self.heads, output_dim)
    ...
    def forward(self, pro1_data, pro2_data, mas1_straight, mas1_flipped, mas2_straight, mas2_flipped, mas1_straight_lengths, mas1_flipped_lengths, mas2_straight_lengths, mas2_flipped_lengths):
        ...
        # Process protein 1 with GAT - same logic as other models
        pro1_x, pro1_edge_index, pro1_batch = pro1_data.x, pro1_data.edge_index, pro1_data.batch
        pro1_nodes = self.pro1_conv1(pro1_x, pro1_edge_index)
        pro1_nodes = self.relu(pro1_nodes)
        # Apply linear layer to each node (instead of global pooling)
        ...
        # Process protein 2 with GAT - same logic as other models  
        pro2_x, pro2_edge_index, pro2_batch = pro2_data.x, pro2_data.edge_index, pro2_data.batch
        pro2_nodes = self.pro2_conv1(pro2_x, pro2_edge_index)
        pro2_nodes = self.relu(pro2_nodes)
        # Apply linear layer to each node (instead of global pooling)
        ...
        for i in range(batch_size):
            ...
            # Combine all elements for this sample: nodes only
            sequence = torch.cat([
                pro1_sample_nodes,
                pro2_sample_nodes,
            ], dim=0)  # Shape: (total_seq_len, transformer_dim)
