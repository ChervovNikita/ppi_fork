...
class ProteinDataset(Dataset):
    ...
    def _get_esm_embeddings(self, sequence_full):
        # Truncate sequence if too long
        seq_len_full = len(sequence_full)
        embeds = []
        for start in range(0, seq_len_full, 1024):
            sequence = sequence_full[start:start+1024]
        ...
        result = torch.cat(embeds, dim=0)
        print(result.shape)
        return embeddings.to('cpu')  # Move back to CPU for compatibility

        # seq_len_full = len(sequence_full)
        # embeds = []
        # for start in range(0, seq_len_full, 1000):
        #     sequence = sequence_full[start:start+1000]
        ...
        #                 )
        #     emb_0 = embedding_rpr.last_hidden_state[0,1:1+seq_len]
        #     embeds.append(emb_0)
        # if not embeds:
        #     return torch.zeros(0, 1024)
        # result = torch.cat(embeds, dim=0)
