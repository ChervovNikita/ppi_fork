...
print(f'len test: {len(testloader)}')
...
for epoch in range(num_epochs):
  ...
  print(f'Epoch [{epoch}/{num_epochs}] [==============================] - val_loss : {loss} - val_mse : {accuracy}')
  if(accuracy < best_mse):
    best_mse = accuracy
    best_acc_epoch = epoch
    torch.save(model.state_dict(), "../human_features/GCN_50.pth") #path to save the model
    print("Model")
  if(loss< min_loss):
    epochs_no_improve = 0
    min_loss = loss
